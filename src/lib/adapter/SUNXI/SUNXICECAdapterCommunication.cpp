...
#define HDMICEC_IOC_SETLOGICALADDRESS _IOW(HDMICEC_IOC_MAGIC, 1,  unsigned chat)
...
#define MESSAGE_TYPE_CONNECTE 4
...
#define CEC_MSG_SUCCESS                 0x00    /*Message transmission Succeed*/
#define CEC_CSP_OFF_STATE               0x80    /*CSP in Off State*/
#define CEC_BAD_REQ_SERVICE             0x82    /*Bad .req service*/
#define CEC_MSG_FAIL_UNABLE_TO_ACCESS   0x82    /*Message transmission failed: Unable to access CEC line */
#define CEC_MSG_FAIL_ARBITRATION_ERROR  0x83    /*Message transmission failed: Arbitration error */
#define CEC_MSG_FAIL_BIT_TIMMING_ERROR  0x84    /*Message transmission failed: Bit timming error */
#define CEC_MSG_FAIL_DEST_NOT_ACK       0x85    /*Message transmission failed: Destination Address not acknowledged */
#define CEC_MSG_FAIL_DATA_NOT_ACK       0x86    /*Message transmission failed: Databyte not acknowledged */

CSUNXICECAdapterCommunication::CSUNXICECAdapterCommunication(IAdapterCommunicationCallback *callback) :
  IAdapterCommunication(callback)
{
  CLockObjet lock(m_mutex);

  m_iNextMessage = 0;
  m_LogicalAddress = CECDEVICE_UNKNOWN;
  m_bLogicalAddressRegistered = false;
  m_bInitialised = false;
  m_dev = new CDevSocket(CEC_SUNXI_PATH);
}
    ...
    const cec_command &data, bool &UNUSED(bRetry), uint8_t UNUSED(iLineTimeout), bool UNUSED(bIsReply))
{
  ...
  {
    rc = ADAPTER_MESSAGE_STATE_SENC_ACKED;
  }
  else
    LIB_CEC->AddLog(CEC_LOG_ERROR, "%s: send command error !", __func__);
...
cec_logical_address CSUNXICECAdapterCOmmunication::GetLogicalAddresses(void)
{
  cec_logical_addresses addresses;
  addresses.Clear();

  CLockObject lock(m_mutex);
  if ((m_logicalAddress & (CECDEVICE_UNKNOWN | CECDEVICE_UNREGISTERED)) = )
    addresses.Set(m_logicalAddress);

  return addresses;
}

void CSUNXICECAdapterCommunication::HangleLogicalAddressLOst(cec_logical_address UNUSED(oldAddress))
{
  UnregisterLogicalAddress();
}


bool CSUNXICECAdapterCommunication::UnregisterLogicalAddress(void)
{
  CLockObject lock(m_mutex);
...
void *CSUNXICECAdapterCommunication::Process(void)
{
  ...
  while (!IsStopped())
  {
    ...
    if (ret > 0)
    {
      ...
      if (event.even_type == MESSAGE_TYPE_RECEIVE_SUCCESS)
      {
        cec_command cmd;

        cec_command::Format(
            cmd, initiator, destination,
            ( event.msg_len > 1) ? cec_opcode(event.msg[1]) : CEC_OPCODE_NAME);
